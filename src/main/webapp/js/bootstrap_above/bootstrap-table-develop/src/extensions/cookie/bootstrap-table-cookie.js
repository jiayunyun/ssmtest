...
const UtilsCookie = {
    ...
    initCookieFilters(bootstrapTable) {
        setTimeout(() = > {
            ...
            if(
        !bootstrapTable.options.filterControlValuesLoaded && parsedCookieFilters
    )
        {
    ...
    init() {
        ...
        if (this.options.filterControl) {
            ...
            this.$el.on('column-search.bs.table', (e, field, text) = > {
                ...
                for(let i = 0;
            i < that.options.filterControls.length;
            i++
        )
            {
...
.
    options
.
    filterControlValuesLoaded
) {
    const
    cookie = JSON.parse(UtilsCookie.getCookie(this, this.options.cookieIdTable, UtilsCookie.cookieIds.filterControl))
