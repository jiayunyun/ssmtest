...
)
{
    ...
    if (this.options.showMultiSort) {
        ...
        this.$el.on('load-success.bs.table', () = > {
            if(
        !isSingleSort && that.options.sortPriority !== null && typeof that.options.sortPriority === 'object' && that.options.sidePagination !== 'server'
    )
        {
            that.onMultipleSort()
        }
        ...
        this.$el.on('column-switch.bs.table', (field, checked) = > {
            for(let i = 0;
        i < that.options.sortPriority.length;
        i++
    )
        {
        ...
        this.$el.on('reset-view.bs.table', () = > {
            if(
        !isSingleSort && that.options.sortPriority !== null && typeof that.options.sortPriority === 'object'
    )
        {
            that.assignSortableArrows()
        }
